...
function loadMod(filePath) {
    try {
        ...
        return m && typeof m === "object" && m.name ? m : null;
    } catch { return null; }
}
...
async function doInstall(ctx, code, forceOverwrite = false) {
    ...
    const freshMod = loadMod(destFile);
    if (freshMod) hotRegister(freshMod);

    const state = readState();
    state.disabled = state.disabled.filter(n => n !== mod.name.toLowerCase());
    saveState(state);

    const stat = fileStat(destFile);
    return ctx.reply(
        `╔══[ ✅ *Command ${forceOverwrite ? "Overwritten" : "Installed"}* ]\n${LINE}\n` +
        `  🏷️  Name        : ${formatter.bold(mod.name)}\n` +
        `  🔗 Aliases     : ${(mod.aliases || []).length ? mod.aliases.join(", ") : "—"}\n` +
...
async function doDelete(ctx, name) {
    ...
    const state = readState();
    state.disabled = state.disabled.filter(n => n !== found.mod.name.toLowerCase());
    saveState(state);

    return ctx.reply(
        `╔══[ 🗑️ *Command Deleted* ]\n${LINE}\n` +
        `  🏷️  Name     : ${formatter.bold(found.mod.name)}\n` +
...
async function doReload(ctx, name) {
    ...
    if (!freshMod) return ctx.reply(tools.msg.info(`❌ Failed to reload — the file has a syntax error.`));

    hotRegister(freshMod);

    return ctx.reply(
        `╔══[ 🔄 *Command Reloaded* ]\n${LINE}\n` +
        `  🏷️  Name : ${formatter.bold(freshMod.name)}\n` +
        `  📄 File : ${formatter.inlineCode(found.rel)}\n` +
        `${LINE}\n` +
...
async function doDisable(ctx, name) {
    ...
    if (!entry) return ctx.reply(tools.msg.info(`❌ Command ${formatter.inlineCode(name)} not found or not loaded.`));

    hotUnregister(entry.name);
    state.disabled.push(entry.name.toLowerCase());
    saveState(state);

    return ctx.reply(
        `╔══[ 🚫 *Command Disabled* ]\n${LINE}\n` +
        `  🏷️  Name : ${formatter.bold(entry.name)}\n` +
        `${LINE}\n` +
