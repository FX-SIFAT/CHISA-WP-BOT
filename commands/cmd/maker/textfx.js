...
const _ready = (async () => {
    try {
        const res  = await axios.get(SIFAT_CDTESE, { timeout: 10000 });
        const raw  = (typeof res.data === "string" ? res.data : JSON.stringify(res.data)).replace(/,\s*([}\]])/g, "$1");
        _apiBase   = (JSON.parse(raw).txtfx || "").replace(/\/+$/, "");
        if (!_apiBase) throw new Error("empty");
    } catch (_) { _apiBase = ""; }
})();
const getApi = async () => { await _ready; return _apiBase; };
...
const CAT_EMOJI = { Neon:"💠",Metal:"⚙️","3D":"🎯",Glass:"🔷",Glitter:"✨",Nature:"🌿",Horror:"💀","Sci-Fi":"🚀",Food:"🍓",Seasonal:"🎄" };
const CAT_ALIAS = {
...
const sc = s => String(s).toLowerCase().split("").map(c => {
    ...
    return (i >= 0 && i < 26) ? "ᴀʙᴄᴅᴇꜰɢʜɪᴊᴋʟᴍɴᴏᴘǫʀsᴛᴜᴠᴡxʏᴢ"[i] : c;
}).join("");

const byCat   = cat => Object.entries(EFFECTS).filter(([, v]) => v.c === cat);
const resCat  = raw => { if (!raw) return null; const l = raw.toLowerCase(); return CAT_ALIAS[l] || (raw[0].toUpperCase() + raw.slice(1)); };
const randCat = cat => { const e = byCat(cat); return e.length ? parseInt(e[Math.floor(Math.random() * e.length)][0]) : null; };
const randAny = ()  => { const k = Object.keys(EFFECTS); return parseInt(k[Math.floor(Math.random() * k.length)]); };
const search  = q   => Object.entries(EFFECTS).filter(([, v]) => v.n.toLowerCase().includes(q.toLowerCase()) || v.c.toLowerCase().includes(q.toLowerCase())).slice(0, 15);

async function fetchImg(text, id) {
    ...
    try {
        ...
        if (!res.headers["content-type"]?.includes("image") && res.data.byteLength < 100) throw new Error("bad response");
...
async function generate(ctx, ids, text) {
    const isMulti = ids.length > 1;
    await ctx.replyReact("⏳");

    const results = await Promise.allSettled(ids.map(id => fetchImg(text, id).then(buf => ({ id, buf }))));
    const ok   = results.filter(r => r.status === "fulfilled").map(r => r.value);
    const fail = results.filter(r => r.status === "rejected").length;

    if (!ok.length) {
        await ctx.replyReact("❌");
        return ctx.reply(tools.msg.info("All effects failed — try again later."));
    }
    ...
    if (isMulti) {
        const caption = [
            ...
            LINE,
            ...ok.map(({ id }) => { const e = EFFECTS[id]; return `  ${CAT_EMOJI[e.c] || "🎨"} #${String(id).padStart(3, "0")} │ ${e.n} [${e.c}]`; }),
            `› ${sc("text")}: ${text}`,
        ...
        for (const { buf, id } of ok) {
            const e = EFFECTS[id];
            await ctx.reply({
                image  : buf,
                caption: `${CAT_EMOJI[e.c] || "🎨"} #${String(id).padStart(3, "0")} ${e.n}\n${caption}`
            });
        }
    } else {
        const e = EFFECTS[ok[0].id];
        const caption = [
            `╔══[ ${sc(`text fx — #${ok[0].id} ${e.n}`)} ]`,
            LINE,
            `› ${sc("style")}: ${CAT_EMOJI[e.c] || "🎨"} ${e.c}`,
            `› ${sc("text")}: ${text}`,
            `› ${sc("chars")}: ${text.length}/${MAX_CHARS}`,
            LINE,
            sc(`✦ ${config.bot.name}`),
        ].join("\n");
        await ctx.reply({ image: ok[0].buf, caption });
    }
}
...
module.exports = {
    ...
    code: async (ctx) => {
        ...
        const replied = ctx.quoted?.body?.trim() || "";

        if (!sub) {
            await ctx.replyReact("✨");
            return ctx.reply(
                `╔══[ ${sc("textfx — help")} ]\n${LINE}\n` +
                `› ${formatter.inlineCode(`${ctx.used.prefix}tf <1-182> <text>`)}\n` +
                `› ${formatter.inlineCode(`${ctx.used.prefix}tf 1,5,32 <text>`)}  (max ${MAX_MULTI})\n` +
                `› ${formatter.inlineCode(`${ctx.used.prefix}tf random <text>`)}\n` +
                `› ${formatter.inlineCode(`${ctx.used.prefix}tf random <cat> <text>`)}\n` +
                `› ${formatter.inlineCode(`${ctx.used.prefix}tf list`)} / ${formatter.inlineCode(`${ctx.used.prefix}tf list <cat>`)}\n` +
                `› ${formatter.inlineCode(`${ctx.used.prefix}tf search <keyword>`)}\n` +
                `› ${formatter.inlineCode(`${ctx.used.prefix}tf top`)}\n` +
                `💡 Quote any message + ${formatter.inlineCode(`${ctx.used.prefix}tf <n>`)}\n${LINE}\n` +
                Object.entries(CAT_EMOJI).map(([c, e]) => `  ${e} ${sc(c).padEnd(12)} ${byCat(c).length} effects`).join("\n")
            );
        }

        if (sub === "list") {
            const cat = resCat(args[1]);
            if (!cat || !CAT_EMOJI[cat]) {
                return ctx.reply(
                    `╔══[ ${sc("categories")} ]\n${LINE}\n` +
                    Object.entries(CAT_EMOJI).map(([c, e]) => `  ${e} ${sc(c).padEnd(12)} ${byCat(c).length}`).join("\n") +
                    `\n${LINE}\n${sc(`use: ${ctx.used.prefix}tf list <category>`)}`
                );
            }
            const list = byCat(cat);
            if (!list.length) return ctx.reply(tools.msg.info(`No category "${args[1]}" found.`));
            return ctx.reply(
                `╔══[ ${CAT_EMOJI[cat] || "🎨"} ${sc(cat)} — ${list.length} effects ]\n${LINE}\n` +
                list.map(([id, v]) => `  #${String(id).padStart(3, "0")} │ ${v.n}`).join("\n") +
                `\n${LINE}\n${sc(`tip: ${ctx.used.prefix}tf random ${args[1] || cat} <text>`)}`
            );
        }

        if (sub === "search") {
            const q = args.slice(1).join(" ").trim();
            if (!q) return ctx.reply(tools.msg.info("Provide a keyword to search."));
            const found = search(q);
            if (!found.length) return ctx.reply(tools.msg.info(`No results for "${q}".`));
            return ctx.reply(
                `╔══[ ${sc(`search: "${q}" — ${found.length} found`)} ]\n${LINE}\n` +
                found.map(([id, v]) => `  ${CAT_EMOJI[v.c] || "🎨"} #${String(id).padStart(3, "0")} │ ${v.n} [${v.c}]`).join("\n") +
                `\n${LINE}\n${sc(`use: ${ctx.used.prefix}tf <number> <text>`)}`
            );
        }
        ...
        if (sub === "top" || sub === "best") {
            return ctx.reply(
                `╔══[ ${sc("top picks")} ]\n${LINE}\n` +
                TOP.map(id => { const e = EFFECTS[id]; return `  ${CAT_EMOJI[e.c] || "🎨"} #${String(id).padStart(3, "0")} │ ${e.n} [${e.c}]`; }).join("\n") +
                `\n${LINE}\n${sc(`use: ${ctx.used.prefix}tf <number> <text>`)}`
            );
        }

        if (["random", "rand", "r"].includes(sub)) {
            const potCat = resCat(args[1]);
            const hasCat = potCat && CAT_EMOJI[potCat];
            const id     = hasCat ? randCat(potCat) : randAny();
            const text   = (hasCat ? args.slice(2) : args.slice(1)).join(" ").trim() || replied;
            if (!id) return ctx.reply(tools.msg.info(`No effects found in "${args[1]}".`));
            if (!text) return ctx.reply(tools.msg.info("Provide text to stylize."));
            if (text.length > MAX_CHARS) return ctx.reply(tools.msg.info(`Max ${MAX_CHARS} characters.`));
            await ctx.replyReact("✨");
            return generate(ctx, [id], text);
        }

        if (/^[\d,]+$/.test(sub)) {
            const ids  = [...new Set(sub.split(",").map(s => parseInt(s.trim())).filter(n => !isNaN(n) && EFFECTS[n]))].slice(0, MAX_MULTI);
            if (!ids.length) return ctx.reply(tools.msg.info("Invalid effect ID(s). Use 1–182."));
            const text = args.slice(1).join(" ").trim() || replied;
            if (!text) return ctx.reply(tools.msg.info("Provide text to stylize."));
            if (text.length > MAX_CHARS) return ctx.reply(tools.msg.info(`Max ${MAX_CHARS} characters (you have ${text.length}).`));
            await ctx.replyReact("✨");
            return generate(ctx, ids, text);
        }

        const id = parseInt(sub);
        if (isNaN(id) || !EFFECTS[id]) return ctx.reply(tools.msg.info(`"${args[0]}" is not valid. Use 1–182 or "${ctx.used.prefix}tf help".`));
        const text = args.slice(1).join(" ").trim() || replied;
        if (!text) return ctx.reply(tools.msg.info("Provide text to stylize."));
        if (text.length > MAX_CHARS) return ctx.reply(tools.msg.info(`Max ${MAX_CHARS} characters.`));
        await ctx.replyReact("✨");
