...
function writeEventFile(filePath, code) {
    ...
    const hasEventing = /Events\.\w+|bot\.ev\.on|\.ev\.on/i.test(code);
    if (!hasEventing) {
        return { ok: false, reason: "File doesn't appear to register any event listener (no `Events.*` or `.ev.on` usage found)." };
    }
    ...
    async code(ctx) {
        ...
        if (sub === "install") {
            ...
            if (!url || !tools.cmd.isUrl(url)) {
                return ctx.reply(
                    tools.msg.info(
                        `Provide a URL or reply to a .js file.\n` +
