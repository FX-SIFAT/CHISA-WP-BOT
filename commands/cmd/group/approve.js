    ...
    code: async (ctx) => {
        ...
        if (ctx.args[0]?.toLowerCase() === "all") {
            try {
                ...
                await ctx.group().approvePendingMembers(jids);
                return await ctx.reply(tools.msg.info(`Approved all ${jids.length} pending member${jids.length !== 1 ? "s" : ""}!`));
            } catch (err) {
        ...
        if (!target.jid)
            return await ctx.reply(
                ...
                tools.msg.generateNotes([
                    `Use ${formatter.inlineCode(`${ctx.used.prefix + ctx.used.command} all`)} to approve all ${pendings.length} pending member${pendings.length !== 1 ? "s" : ""}.`
                ])
            );
        ...
        try {
            ...
            await ctx.reply(tools.msg.info("Member approved successfully!"));
        } catch (err) {
